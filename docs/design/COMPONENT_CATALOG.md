...
| Cluster-Set tree cards | feature-local components | Show saved root/child Cluster-Sets, status, lineage, active counts, collapsible metadata and allowed actions including batch selection actions menu, full-state duplicate and Summary-regeneration dialog entry | `frontend/src/App.tsx`, `frontend/src/App.css` | Cluster-Sets tab | ClusterSet data, parent/child relation, selection state, per-card metadata collapse state | completed/running/deleted-history, selected/unselected, metadata expanded/collapsed, load/refine/summary-dialog/duplicate/cancel/delete/batch-delete | Card headings, checkbox labels include set names, progress, `aria-expanded` tree and metadata toggles, labeled actions select, disabled invalid actions | App tests and browser evidence | Feature-local tree and batch toolbar; parameter comparison is not implemented | Extend in place for accepted Cluster-Sets tab actions; extract only after another screen needs tree cards | active | not-applicable |
